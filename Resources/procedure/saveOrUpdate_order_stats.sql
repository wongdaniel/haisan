...
IF (@id > 0) BEGIN
	IF NOT EXISTS(SELECT * FROM tb_order_stats WHERE id = @id) BEGIN
		...
		RETURN -1	
	END ELSE BEGIN
		UPDATE tb_order_stats SET [order] = @order,type_of_process = @type_of_process,[image] = @image,unit = @unit,
			total_number = @total_number,unit_price = @unit_price,amount_of_money = @amount_of_money
			WHERE id = @id
		RETURN 0
	END
END ELSE BEGIN
		INSERT INTO tb_order_stats([order],type_of_process,[image],unit,total_number,unit_price,amount_of_money)
			VALUES(@order,@type_of_process,@image,@unit,@total_number,@unit_price,@amount_of_money)
		RETURN 0
END
